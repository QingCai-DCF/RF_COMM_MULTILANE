...
if (-not $EvidenceDir) {
    $EvidenceDir = Join-Path $repoRoot "evidence\generated\hw_preflight\run_lane0_raw_matrix_safe_$stamp"
} elseif (-not [System.IO.Path]::IsPathRooted($EvidenceDir)) {
    $EvidenceDir = Join-Path $repoRoot $EvidenceDir
}
New-Item -ItemType Directory -Force -Path $EvidenceDir | Out-Null

$summaryLog = Join-Path $EvidenceDir "run_lane0_raw_matrix_safe.summary.txt"
...
"RUN_LANE0_RAW_MATRIX_SAFE_BEGIN $(Get-Date -Format o)" | Set-Content -LiteralPath $summaryLog -Encoding ascii
...
if (-not $hardwareRequested) {
    ...
    Write-SummaryLine "RUN_LANE0_RAW_MATRIX_SAFE_STATUS=REFUSED_NO_ALLOW_HARDWARE"
    Write-SummaryLine "RUN_LANE0_RAW_MATRIX_SAFE_END $(Get-Date -Format o)"
    exit 0
}
...
if ($authExit -ne 0) {
    ...
    Write-SummaryLine "NO_HARDWARE_ACTIONS_EXECUTED=1"
    Write-SummaryLine "RUN_LANE0_RAW_MATRIX_SAFE_STATUS=AUTHORIZATION_MISSING"
    Write-SummaryLine "RUN_LANE0_RAW_MATRIX_SAFE_END $(Get-Date -Format o)"
    exit 2
}
...
if ($AutoBuildPsElfPerTrigger.IsPresent) {
    $innerArgs += "-AutoBuildPsElfPerTrigger"
}
if ($StopOnFail.IsPresent) {
    $innerArgs += "-StopOnFail"
}
...
try {
    ...
    $innerExit = Invoke-LoggedProcess `
        ...
        -Timeout ([Math]::Max(600, ($PerRunTimeoutSeconds * [Math]::Max(1, $TriggerModes.Count)) + 300))
...
Write-SummaryLine "RUN_LANE0_RAW_MATRIX_SAFE_STATUS=$(if ($innerExit -eq 0 -and $shutdownOk) { 'PASS' } else { 'FAIL' })"
Write-SummaryLine "RUN_LANE0_RAW_MATRIX_SAFE_END $(Get-Date -Format o)"

if (-not $shutdownOk) {
    exit $(if ($shutdownExit -ne 0) { $shutdownExit } else { 32 })
}
if ($innerExit -ne 0) {
    exit $innerExit
}
