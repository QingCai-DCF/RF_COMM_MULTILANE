#!/usr/bin/env python3
"""Directly reverify P8D PASS bindings changed by the P10 endpoint reset wiring."""
...
OUT = ROOT / "evidence/generated/p10_p8d_source_reverification"
...
STAGE = "P10_DUAL_ENDPOINT_SOURCE_REVERIFY"
TEST_ID = "P10-P8D-SOURCE-BINDING-REVERIFICATION"
...
def main() -> int:
    ...
    if missing_tools:
        ...
        return 2
    source_paths = sorted({path for spec in TESTS for path in spec["sources"]} | {path for paths in BOUND_ARTIFACTS.values() for path in paths})
    ...
    status = "PASS" if config_result["returncode"] == 0 and all(item["status"] == "PASS" for item in simulations) else "FAIL"
    summary = {
        "schema_version": 1,
        "stage": STAGE,
        "test_id": TEST_ID,
        ...
        "claim_boundary": (
            "Refreshes only L2-ARQ-002 and L2-SACK-002 current-source artifact bindings after the P10 "
            "ACK-state reset wiring change. It does not promote any hardware, Z7020, rotation, D17, or final-product scope."
        ),
        "requirements": {
