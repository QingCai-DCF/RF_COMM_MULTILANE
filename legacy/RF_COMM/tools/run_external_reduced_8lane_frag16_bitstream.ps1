...
if ([string]::IsNullOrWhiteSpace($RouteDir)) {
    $latestRoute = Get-ChildItem -LiteralPath $reportsDir -Directory -Filter "build_external_reduced_8lane_frag16_route_*" |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
    if ($null -eq $latestRoute) {
        throw "No build_external_reduced_8lane_frag16_route_* directory found"
    }
...
if (-not (Test-Path -LiteralPath $routeDcp -PathType Leaf)) {
    throw "Missing reduced 8-lane fragment=16 post-route checkpoint: $routeDcp"
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$prefix = "external_reduced_8lane_frag16_bitstream_$stamp"
...
$tclScript = Join-Path $scriptDir "write_external_reduced_8lane_frag16_bitstream.tcl"
$bitstream = Join-Path $outDir "external_reduced_8lane_frag16_candidate.bit"
...
$env:EXT8F16_BIT_ROUTE_DIR = $RouteDir
$env:EXT8F16_BIT_OUT_DIR = $outDir
$env:EXT8F16_BIT_NO_HARDWARE_PROGRAMMING = "1"
$env:EXT8F16_BIT_NO_UART_WRITE = "1"
$env:EXT8F16_BIT_NO_TFDU_DRIVE = "1"
$env:EXT8F16_BIT_ETHERNET_DEFERRED = "1"
...
Write-Host "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_START meta=$meta"
Write-Host "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_COMMAND $VivadoPath $($arguments -join ' ')"
...
try {
    ...
    Write-Host "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_AFFINITY_SET mask=0x$($AffinityMask.ToString('X'))"
} catch {
    Add-Content -LiteralPath $meta -Value "affinity_set_failed=$($_.Exception.Message)" -Encoding ascii
    Write-Host "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_AFFINITY_SET_FAILED $($_.Exception.Message)"
}
...
Write-Host "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_EXIT_CODE=$exitCode"
exit $exitCode
