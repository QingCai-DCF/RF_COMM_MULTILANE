...
$summaryLog = Join-Path $reportsDir "build_psps_rx_only_elf_$stamp.summary.txt"
$outLog = Join-Path $reportsDir "build_psps_rx_only_elf_$stamp.out.log"
$errLog = Join-Path $reportsDir "build_psps_rx_only_elf_$stamp.err.log"
...
function Set-RxOnlyBuildEnv {
    ...
    $env:PSPS_RX_LANE_MASK = $LaneMask
    $env:PSPS_POLL_SLEEP_US = [string]$PollSleepUs
    $env:IR_TX_POLL_US = "1"
    $env:IR_HW_MAX_PACKET_BYTES = [string]$MaxPacketBytes
    $env:IR_HW_RX_TRANSFER_BYTES = [string]$RxTransferBytes
}

"BUILD_PSPS_RX_ONLY_ELF_BEGIN $(Get-Date -Format o)" | Out-File -FilePath $summaryLog -Encoding ascii
...
Write-SummaryLine "RAW_BYTES=$($PayloadBytes + 8)"
Write-SummaryLine "PSPS_RX_ONLY=1"
Write-SummaryLine "PSPS_TX_ONLY=0"
Write-SummaryLine "PSPS_TDM_BIDIR=0"
Write-SummaryLine "STAGE_SECONDS=$StageSeconds"
Write-SummaryLine "STATS_INTERVAL_US=$StatsIntervalUs"
Write-SummaryLine "POLL_SLEEP_US=$PollSleepUs"
...
if (Test-Path -LiteralPath $elfPath) {
    $preHash = Get-FileHash -Algorithm SHA256 -LiteralPath $elfPath
    Write-SummaryLine "PRE_ELF=$elfPath"
    Write-SummaryLine "PRE_ELF_SHA256=$($preHash.Hash)"
    Write-SummaryLine "PRE_ELF_SIZE=$((Get-Item -LiteralPath $elfPath).Length)"
} else {
    Write-SummaryLine "PRE_ELF=MISSING"
}

if (($PayloadBytes + 8) -gt $MaxPacketBytes) {
    Write-SummaryLine "BUILD_BLOCKED_RAW_BYTES_EXCEED_PACKET=1"
    Write-SummaryLine "BUILD_PSPS_RX_ONLY_ELF_END $(Get-Date -Format o)"
