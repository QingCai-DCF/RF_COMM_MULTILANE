...
$prefix = "build_external_reduced_2lane_route_$stamp"
...
Write-Host "EXTERNAL_REDUCED_2LANE_ROUTE_START meta=$meta"
Write-Host "EXTERNAL_REDUCED_2LANE_ROUTE_COMMAND $VivadoPath $($arguments -join ' ')"
...
try {
    ...
    Write-Host "EXTERNAL_REDUCED_2LANE_ROUTE_AFFINITY_SET mask=0x$($AffinityMask.ToString('X'))"
} catch {
    Add-Content -LiteralPath $meta -Value "affinity_set_failed=$($_.Exception.Message)" -Encoding ascii
    Write-Host "EXTERNAL_REDUCED_2LANE_ROUTE_AFFINITY_SET_FAILED $($_.Exception.Message)"
}
...
Write-Host "EXTERNAL_REDUCED_2LANE_ROUTE_EXIT_CODE=$exitCode"
exit $exitCode
