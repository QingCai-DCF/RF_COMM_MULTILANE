...
def latest_meta() -> Path | None:
    metas = sorted(
        REPORTS.glob("external_reduced_8lane_frag16_bitstream_*.meta.txt"),
...
def main() -> int:
    ...
    if meta_file is None:
        raise FileNotFoundError("No external_reduced_8lane_frag16_bitstream_*.meta.txt found")
    ...
    timing_rpt = out_dir / "external_reduced_8lane_frag16_candidate_timing_summary.rpt" if out_dir else None
    util_rpt = out_dir / "external_reduced_8lane_frag16_candidate_utilization.rpt" if out_dir else None
    route_rpt = out_dir / "external_reduced_8lane_frag16_candidate_route_status.rpt" if out_dir else None
    drc_rpt = out_dir / "external_reduced_8lane_frag16_candidate_drc.rpt" if out_dir else None
    io_rpt = out_dir / "external_reduced_8lane_frag16_candidate_io.rpt" if out_dir else None
    dcp = out_dir / "external_reduced_8lane_frag16_candidate_post_route.dcp" if out_dir else None
    ...
    marker_ok = "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_READY" in vivado_text
    no_hw = all(
        marker in vivado_text
        for marker in [
            "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_NO_HARDWARE_PROGRAMMING=1",
            "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_NO_UART_WRITE=1",
            "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_NO_TFDU_DRIVE=1",
        ]
    )
    safe_meta = all(meta.get(key) == "1" for key in ["no_hardware_programming", "no_uart_write", "no_tfdu_drive"])
    ethernet_deferred = "EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM_ETHERNET_DEFERRED=1" in vivado_text
    profile_ok = (
        meta.get("lane_count") == "8"
        and meta.get("fragment_bytes") == "16"
        and meta.get("max_packet_bytes") == "64"
    ...
    raw_target_capacity = profile_ok and int(meta["lane_count"]) == 8

    if bit_ok and report_ok and marker_ok and profile_ok and timing_pass and routing_clean and drc_no_critical and no_hw and safe_meta and vivado_ok:
        overall = "PASS_OFFLINE_8LANE_FRAG16_BITSTREAM_READY_REVIEW_REQUIRED"
    else:
    ...
    items = [
        ...
        BitstreamItem("PROFILE", "PASS" if profile_ok else "FAIL", rel(meta_file), f"lane={meta.get('lane_count')} fragment={meta.get('fragment_bytes')} max_packet={meta.get('max_packet_bytes')} dbg={meta.get('stream_phy_dbg_select')}"),
        BitstreamItem("RAW-CAPACITY", "PASS_RAW_32_16" if raw_target_capacity else "FAIL", rel(meta_file), "8 lanes at 4 Mbit/s raw PHY gives 32 Mbit/s half-duplex and 16 Mbit/s per direction with 4+4 full-duplex partition."),
    ...
    md_path = REPORTS / "external_reduced_8lane_frag16_bitstream_current.md"
    json_path = REPORTS / "external_reduced_8lane_frag16_bitstream_current.json"
    csv_path = REPORTS / "external_reduced_8lane_frag16_bitstream_current.csv"
    ...
    md = [
        "# External Reduced 8-Lane Fragment-16 Candidate Bitstream",
        "",
        ...
        f"- Profile: `lane=8`, `fragment=16`, `max_packet=64`, `stream_phy_dbg_select=6`.",
        ...
        "",
        f"RF_COMM_EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM overall={overall} bitstream={rel(bitstream)} sha256={sha256(bitstream)} wns={timing['wns_ns']} whs={timing['whs_ns']} route_errors={route.get('routing_errors')} raw_half_mbps=32.000 raw_fdx_per_dir_mbps=16.000",
    ]
    md_path.write_text("\n".join(md) + "\n", encoding="utf-8")
    ...
    payload = {
        ...
        "raw_target_capacity": raw_target_capacity,
    ...
    print(
        f"RF_COMM_EXTERNAL_REDUCED_8LANE_FRAG16_BITSTREAM overall={overall} "
        f"bitstream={rel(bitstream)} sha256={sha256(bitstream)} "
        f"wns={timing['wns_ns']} whs={timing['whs_ns']} route_errors={route.get('routing_errors')} "
        "raw_half_mbps=32.000 raw_fdx_per_dir_mbps=16.000"
    )
    return 0 if overall == "PASS_OFFLINE_8LANE_FRAG16_BITSTREAM_READY_REVIEW_REQUIRED" else 1
