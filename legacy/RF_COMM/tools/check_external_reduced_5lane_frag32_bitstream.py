...
def latest_meta() -> Path | None:
    metas = sorted(
        REPORTS.glob("external_reduced_5lane_frag32_bitstream_*.meta.txt"),
...
def main() -> int:
    ...
    if meta_file is None:
        raise FileNotFoundError("No external_reduced_5lane_frag32_bitstream_*.meta.txt found")
    ...
    timing_rpt = out_dir / "external_reduced_5lane_frag32_candidate_timing_summary.rpt" if out_dir else None
    util_rpt = out_dir / "external_reduced_5lane_frag32_candidate_utilization.rpt" if out_dir else None
    route_rpt = out_dir / "external_reduced_5lane_frag32_candidate_route_status.rpt" if out_dir else None
    drc_rpt = out_dir / "external_reduced_5lane_frag32_candidate_drc.rpt" if out_dir else None
    io_rpt = out_dir / "external_reduced_5lane_frag32_candidate_io.rpt" if out_dir else None
    dcp = out_dir / "external_reduced_5lane_frag32_candidate_post_route.dcp" if out_dir else None
    ...
    marker_ok = "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_READY" in vivado_text
    no_hw = all(
        marker in vivado_text
        for marker in [
            "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_NO_HARDWARE_PROGRAMMING=1",
            "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_NO_UART_WRITE=1",
            "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_NO_TFDU_DRIVE=1",
        ]
    )
    safe_meta = all(meta.get(key) == "1" for key in ["no_hardware_programming", "no_uart_write", "no_tfdu_drive"])
    ethernet_deferred = "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_ETHERNET_DEFERRED=1" in vivado_text
    profile_ok = (
        meta.get("lane_count") == "5"
    ...
    if bit_ok and report_ok and marker_ok and profile_ok and timing_pass and routing_clean and drc_no_critical and no_hw and safe_meta and vivado_ok:
        overall = "PASS_OFFLINE_5LANE_FRAG32_BITSTREAM_READY_REVIEW_REQUIRED"
    else:
    ...
    md_path = REPORTS / "external_reduced_5lane_frag32_bitstream_current.md"
    json_path = REPORTS / "external_reduced_5lane_frag32_bitstream_current.json"
    csv_path = REPORTS / "external_reduced_5lane_frag32_bitstream_current.csv"
    ...
    md = [
        "# External Reduced 5-Lane Fragment-32 Candidate Bitstream",
        "",
        ...
        f"- Profile: `lane=5`, `fragment=32`, `max_packet=128`, `stream_phy_dbg_select=6`.",
        ...
        "",
        f"RF_COMM_EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM overall={overall} bitstream={rel(bitstream)} sha256={sha256(bitstream)} wns={timing['wns_ns']} whs={timing['whs_ns']} route_errors={route.get('routing_errors')}",
    ]
    md_path.write_text("\n".join(md) + "\n", encoding="utf-8")
    ...
    print(
        f"RF_COMM_EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM overall={overall} "
    ...
    return 0 if overall == "PASS_OFFLINE_5LANE_FRAG32_BITSTREAM_READY_REVIEW_REQUIRED" else 1
