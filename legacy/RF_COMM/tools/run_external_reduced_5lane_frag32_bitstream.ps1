...
if ([string]::IsNullOrWhiteSpace($RouteDir)) {
    $latestRoute = Get-ChildItem -LiteralPath $reportsDir -Directory -Filter "build_external_reduced_5lane_frag32_route_*" |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
    if ($null -eq $latestRoute) {
        throw "No build_external_reduced_5lane_frag32_route_* directory found"
    }
...
if (-not (Test-Path -LiteralPath $routeDcp -PathType Leaf)) {
    throw "Missing reduced 5-lane fragment=32 post-route checkpoint: $routeDcp"
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$prefix = "external_reduced_5lane_frag32_bitstream_$stamp"
...
$tclScript = Join-Path $scriptDir "write_external_reduced_5lane_frag32_bitstream.tcl"
$bitstream = Join-Path $outDir "external_reduced_5lane_frag32_candidate.bit"
...
$env:EXT5F32_BIT_ROUTE_DIR = $RouteDir
$env:EXT5F32_BIT_OUT_DIR = $outDir
$env:EXT5F32_BIT_NO_HARDWARE_PROGRAMMING = "1"
$env:EXT5F32_BIT_NO_UART_WRITE = "1"
$env:EXT5F32_BIT_NO_TFDU_DRIVE = "1"
$env:EXT5F32_BIT_ETHERNET_DEFERRED = "1"
...
$metaLines = @(
    ...
    "bitstream=$($bitstream.Replace($repoRoot + '\', ''))",
    "lane_count=5",
    "fragment_bytes=32",
    "max_packet_bytes=128",
    "stream_phy_dbg_select=6",
...
Write-Host "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_START meta=$meta"
Write-Host "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_COMMAND $VivadoPath $($arguments -join ' ')"
...
try {
    ...
    Write-Host "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_AFFINITY_SET mask=0x$($AffinityMask.ToString('X'))"
} catch {
    Add-Content -LiteralPath $meta -Value "affinity_set_failed=$($_.Exception.Message)" -Encoding ascii
    Write-Host "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_AFFINITY_SET_FAILED $($_.Exception.Message)"
}
...
Write-Host "EXTERNAL_REDUCED_5LANE_FRAG32_BITSTREAM_EXIT_CODE=$exitCode"
exit $exitCode
