...
$prefix = "build_external_reduced_4lane_route_$stamp"
...
if ($LaneCount -ne 4) {
    throw "This wrapper is for the reduced 4-lane route build; got LaneCount=$LaneCount"
}
...
Write-Host "EXTERNAL_REDUCED_4LANE_ROUTE_START meta=$meta"
Write-Host "EXTERNAL_REDUCED_4LANE_ROUTE_COMMAND $VivadoPath $($arguments -join ' ')"
...
try {
    ...
    Write-Host "EXTERNAL_REDUCED_4LANE_ROUTE_AFFINITY_SET mask=0x$($AffinityMask.ToString('X'))"
} catch {
    Add-Content -LiteralPath $meta -Value "affinity_set_failed=$($_.Exception.Message)" -Encoding ascii
    Write-Host "EXTERNAL_REDUCED_4LANE_ROUTE_AFFINITY_SET_FAILED $($_.Exception.Message)"
}
...
Write-Host "EXTERNAL_REDUCED_4LANE_ROUTE_EXIT_CODE=$exitCode"
exit $exitCode
