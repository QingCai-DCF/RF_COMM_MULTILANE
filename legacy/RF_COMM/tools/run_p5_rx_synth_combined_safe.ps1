...
$summaryLog = Join-Path $reportsDir "P5_02_rx_synth_combined_safe_$stamp.summary.txt"
$xsctOutLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.xsct.out.log"
$xsctErrLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.xsct.err.log"
$hostOutLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.host.out.log"
$hostErrLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.host.err.log"
$preflightLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.preflight.log"
$shutdownOutLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.shutdown.out.log"
$shutdownErrLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.shutdown.err.log"
$shutdownLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.shutdown.log"
$transcript = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.transcript.log"
...
"P5_RX_SYNTH_COMBINED_SAFE_BEGIN $(Get-Date -Format o)" | Out-File -FilePath $summaryLog -Encoding ascii
...
if (-not $SkipSoftRecovery -and (Test-Path -LiteralPath $softRecoveryScript)) {
    ...
    $softOutLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.soft_recovery.out.log"
    $softErrLog = Join-Path $reportsDir "P5_02_rx_synth_combined_$stamp.soft_recovery.err.log"
...
if (-not $SkipPreflight) {
    ...
    if ($preflightText -notmatch "HW_PREFLIGHT_RESULT PASS" -or $preflightText -notmatch "HW_PREFLIGHT_ZYNQ") {
        ...
        Write-SummaryLine "P5_RX_SYNTH_COMBINED_SAFE_END $(Get-Date -Format o)"
        exit 20
    }
}
...
if (Test-Path -LiteralPath $transcript) {
    ...
    foreach ($line in (($transcriptText -split "`r?`n") | Where-Object { $_ -match "UART_OPERATOR_P5_RX_SYNTH_COMBINED_PASS|UART_OPERATOR_P5_RX_SYNTH_COMBINED_FAILURE|UARTOP_RESULT command=TEST|RESULT test_id=|READ dma_obs|READ failure_counters|READ counters" })) {
...
if (Test-Path -LiteralPath $transcript) {
    $transcriptText = Get-Content -LiteralPath $transcript -Raw -ErrorAction SilentlyContinue
    if ($transcriptText -match "UART_OPERATOR_P5_RX_SYNTH_COMBINED_PASS=1") {
        exit 0
    }
}
exit 30
