...
module tb_ir_stream_bidir_single_lane;
  ...
  task automatic send_a_packet;
    begin
      ...
      a_tx_valid = 1'b0;
      a_tx_last  = 1'b0;
      $display("STREAM_A_TO_B_QUEUED t=%0t len=%0d", $time, AB_LEN);
    end
  endtask
  ...
  task automatic send_b_packet;
    begin
      ...
      b_tx_valid = 1'b0;
      b_tx_last  = 1'b0;
      $display("STREAM_B_TO_A_QUEUED t=%0t len=%0d", $time, BA_LEN);
    end
  endtask
  ...
  always @(negedge clk) begin
    if (!rst_n) begin
      ...
      a_rx_ready <= (($time / 1000) % 7) != 3;
      b_rx_ready <= (($time / 1000) % 9) != 4;
    end
  end
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (a_tx_overflow || b_tx_overflow || a_tx_exhaust || b_tx_exhaust ||
        ...
        $fatal(1, "Unexpected stream protocol error t=%0t dbg_a=%08x dbg_b=%08x", $time, debug_a, debug_b);
      end

      if (b_rx_valid && b_rx_ready) begin
        if (ab_ok) $fatal(1, "A-to-B delivered more than once");
        ...
        if (b_rx_last) begin
          ...
          ab_ok <= 1'b1;
          $display("STREAM_A_TO_B_OK t=%0t bytes=%0d", $time, b_rx_count + 1);
        end
      end

      if (a_rx_valid && a_rx_ready) begin
        if (ba_ok) $fatal(1, "B-to-A delivered more than once");
        if (a_rx_count >= BA_LEN) $fatal(1, "B-to-A too many bytes");
        if (a_rx_data !== ba_payload[a_rx_count]) begin
          $fatal(1, "B-to-A byte mismatch idx=%0d exp=%02x got=%02x",
                 a_rx_count, ba_payload[a_rx_count], a_rx_data);
        end
        a_rx_count <= a_rx_count + 1;
        if (a_rx_last) begin
          if ((a_rx_count + 1) != BA_LEN) $fatal(1, "B-to-A length mismatch");
          ba_ok <= 1'b1;
          $display("STREAM_B_TO_A_OK t=%0t bytes=%0d", $time, a_rx_count + 1);
        end
      end
    end
  end
  ...
  initial begin
    ...
    for (int k = 0; k < BA_LEN; k++) ba_payload[k] = byte'((k * 11 + 8'h5a) & 8'hff);
    ...
    repeat (3000000) begin
      ...
      if (ab_ok && ba_ok && (a_tx_done_count == 1) && (b_tx_done_count == 1)) begin
        $display("IR_STREAM_BIDIR_SINGLE_LANE_PASS a_to_b=%0d b_to_a=%0d", AB_LEN, BA_LEN);
        $finish;
      end
    end

    $fatal(1, "Timeout waiting for stream bidir pass ab_ok=%0d ba_ok=%0d a_done=%0d b_done=%0d dbg_a=%08x dbg_b=%08x",
           ab_ok, ba_ok, a_tx_done_count, b_tx_done_count, debug_a, debug_b);
  end
endmodule
