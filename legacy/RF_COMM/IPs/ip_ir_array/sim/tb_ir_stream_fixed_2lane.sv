...
module tfdu_edge_channel_model #(
...
module tb_ir_stream_fixed_2lane;
  ...
  generate
    for (li = 0; li < LANE_COUNT; li = li + 1) begin : g_fixed_optical
      tfdu_edge_channel_model #(
        .DELAY_CYCLES(10),
        .PULSE_CYCLES(15)
      ) ch_b_to_a (
        .clk(clk),
        .rst_n(rst_n),
        .tx_in(b_ir_tx_out[li]),
        .rx_out_n(a_ir_rx_in[li])
      );
      ...
      tfdu_edge_channel_model #(
        ...
        .clk(clk),
        .rst_n(rst_n),
        .tx_in(a_ir_tx_out[li]),
        .rx_out_n(b_ir_rx_in[li])
      );
    end
  endgenerate
  ...
  task automatic send_a_packet;
    begin
      ...
      a_tx_valid = 1'b0;
      a_tx_last  = 1'b0;
      $display("STREAM_2LANE_A_TO_B_QUEUED t=%0t len=%0d", $time, PKT_LEN);
    end
  endtask
  ...
  task automatic send_b_packet;
    begin
      ...
      b_tx_valid = 1'b0;
      b_tx_last  = 1'b0;
      $display("STREAM_2LANE_B_TO_A_QUEUED t=%0t len=%0d", $time, PKT_LEN);
    end
  endtask
    ...
    if (!rst_n) begin
      ...
      for (int l = 0; l < LANE_COUNT; l++) begin
        if (!a_lane_tx_busy_q[l] && a_lane_tx_busy[l]) begin
          ...
          $display("STREAM_2LANE_A_LANE_START t=%0t lane=%0d count=%0d", $time, l, a_lane_starts[l] + 1);
        end
        if (!b_lane_tx_busy_q[l] && b_lane_tx_busy[l]) begin
          b_lane_starts[l] <= b_lane_starts[l] + 1;
          $display("STREAM_2LANE_B_LANE_START t=%0t lane=%0d count=%0d", $time, l, b_lane_starts[l] + 1);
        end
      end
      ...
      if (a_tx_overflow || b_tx_overflow || a_tx_exhaust || b_tx_exhaust ||
        ...
        $fatal(1, "Unexpected 2lane stream error t=%0t dbg_a=%08x dbg_b=%08x", $time, debug_a, debug_b);
      end

      if (b_rx_valid && b_rx_ready) begin
        if (ab_ok) $fatal(1, "A-to-B 2lane delivered more than once");
        if (b_rx_count >= PKT_LEN) $fatal(1, "A-to-B 2lane too many bytes");
        if (b_rx_data !== ab_payload[b_rx_count]) begin
          $fatal(1, "A-to-B 2lane byte mismatch idx=%0d exp=%02x got=%02x",
                 b_rx_count, ab_payload[b_rx_count], b_rx_data);
        end
        b_rx_count <= b_rx_count + 1;
        if (b_rx_last) begin
          if ((b_rx_count + 1) != PKT_LEN) $fatal(1, "A-to-B 2lane length mismatch");
          ab_ok <= 1'b1;
          $display("STREAM_2LANE_A_TO_B_OK t=%0t bytes=%0d", $time, b_rx_count + 1);
        end
      end

      if (a_rx_valid && a_rx_ready) begin
        if (ba_ok) $fatal(1, "B-to-A 2lane delivered more than once");
        if (a_rx_count >= PKT_LEN) $fatal(1, "B-to-A 2lane too many bytes");
        if (a_rx_data !== ba_payload[a_rx_count]) begin
          $fatal(1, "B-to-A 2lane byte mismatch idx=%0d exp=%02x got=%02x",
                 a_rx_count, ba_payload[a_rx_count], a_rx_data);
        end
        a_rx_count <= a_rx_count + 1;
        if (a_rx_last) begin
          if ((a_rx_count + 1) != PKT_LEN) $fatal(1, "B-to-A 2lane length mismatch");
          ba_ok <= 1'b1;
          $display("STREAM_2LANE_B_TO_A_OK t=%0t bytes=%0d", $time, a_rx_count + 1);
        end
      end
    end
  end
  ...
  ir_stream_array_top #(
    ...
    .clk_phy(clk), .rst_n(rst_n), .enable(enable_a), .session_id(16'h2442),
  ...
  ir_stream_array_top #(
    ...
    .clk_phy(clk), .rst_n(rst_n), .enable(enable_b), .session_id(16'h2442),
    .lane_enable_mask(2'b11), .rx_lane_enable_mask(2'b11),
    ...
    repeat (4000000) begin
      ...
      if (ab_ok && ba_ok && (a_tx_done_count == 1) && (b_tx_done_count == 1)) begin
        ...
        if (all_a_lanes_used && all_b_lanes_used) begin
          $display("IR_STREAM_FIXED_2LANE_PASS bytes_each_direction=%0d", PKT_LEN);
          $finish;
        end
      end
    end

    $fatal(1, "Timeout waiting for fixed 2lane pass ab=%0d ba=%0d a_done=%0d b_done=%0d starts_a=%0d/%0d starts_b=%0d/%0d dbg_a=%08x dbg_b=%08x",
           ab_ok, ba_ok, a_tx_done_count, b_tx_done_count,
           a_lane_starts[0], a_lane_starts[1],
           b_lane_starts[0], b_lane_starts[1],
           debug_a, debug_b);
  end
endmodule
