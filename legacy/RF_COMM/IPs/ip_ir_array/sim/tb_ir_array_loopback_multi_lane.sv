...
module tb_ir_array_loopback_multi_lane;
  ...
  task automatic send_packet(input int pkt);
    begin
      ...
      a_tx_last  = 1'b0;
      sent_packets++;
      $display("A_MULTI_PACKET_QUEUED t=%0t pkt=%0d bytes=%0d", $time, pkt, MAX_PACKET_BYTES);
    end
  endtask
    ...
    if (!rst_n) begin
      ...
      for (int l = 0; l < LANE_COUNT; l++) begin
        if (!a_busy_q[l] && a_lane_tx_busy_dbg[l]) begin
          ...
          $display("A_LANE_TX_START t=%0t lane=%0d starts=%0d", $time, l, a_lane_starts[l] + 1);
        end
      end
      ...
      if (a_tx_done_pulse) begin
        ...
        $display("A_MULTI_TX_DONE t=%0t count=%0d", $time, tx_done_count + 1);
      end

      if (b_rx_done_pulse) begin
        b_done_count <= b_done_count + 1;
        $display("B_MULTI_RX_DONE t=%0t count=%0d", $time, b_done_count + 1);
      end

      if (b_rx_valid && b_rx_ready) begin
        if (rx_packet_idx >= PACKET_COUNT) begin
          $fatal(1, "Received unexpected extra multi-lane byte data=%02x", b_rx_data);
        end
        ...
        if (b_rx_last) begin
          ...
          $display("B_MULTI_PAYLOAD_OK t=%0t pkt=%0d bytes=%0d", $time, rx_packet_idx, rx_byte_idx + 1);
  ...
  initial begin
    ...
    enable_a = 1'b0;
    enable_b = 1'b0;
    session_id_a = 16'h4567;
    session_id_b = 16'h4567;
    lane_mask_a = '1;
    lane_mask_b = '1;
    a_tx_data = 8'h00;
    a_tx_valid = 1'b0;
    ...
    for (int p = 0; p < PACKET_COUNT; p++) begin
      for (int k = 0; k < MAX_PACKET_BYTES; k++) begin
        expected[p][k] = byte'(8'h10 + (p * 8'h31) + k);
      end
    end
  ...
  initial begin
    repeat (20000000) begin
      ...
      if (rst_n && (a_tx_error_overflow || a_tx_error_retry_exhausted ||
        ...
        $fatal(1,
          "Multi-lane loopback error flags: tx_overflow=%0b tx_retry_exhausted=%0b rx_hdr=%0b rx_proto=%0b rx_overflow=%0b rx_crc=%0b rx_overrun=%0b",
      ...
      if (sender_done && (sent_packets == PACKET_COUNT) && (rx_packet_idx == PACKET_COUNT) &&
        ...
        $display("LOOPBACK_MULTI_LANE_PASS packets=%0d bytes=%0d max_busy_lanes=%0d max_inflight_frags=%0d lane_starts=%0d,%0d,%0d,%0d",
    ...
    $fatal(1,
      "Timeout waiting for multi-lane packet reception sent=%0d rx_pkt=%0d rx_byte=%0d rx_total=%0d tx_done=%0d b_done=%0d active=%0b loading=%0b rx_ctx=%0b complete=%0b",
