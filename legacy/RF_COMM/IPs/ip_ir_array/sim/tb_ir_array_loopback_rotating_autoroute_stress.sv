...
module tb_ir_array_loopback_rotating_autoroute_stress;
  ...
  localparam int ROTATION_COUNT   = 10;
  localparam int PACKET_COUNT     = ROTATION_COUNT * SECTORS_PER_REV;
  ...
  logic [1:0] ab_map [0:LANE_COUNT-1]; // B RX lane i observes A TX lane ab_map[i].
  logic [1:0] ba_map [0:LANE_COUNT-1]; // A RX lane i observes B TX lane ba_map[i].
  ...
  always @(negedge clk) begin
    if (!rst_n) begin
      ...
      a_rx_ready  <= 1'b1;
      b_rx_ready  <= !((ready_cycle % 23) == 11 || (ready_cycle % 47) == 17);
    end
  end
      ...
      if ((current_packet_idx >= 0) && (current_packet_idx < PACKET_COUNT)) begin
        for (int l = 0; l < LANE_COUNT; l++) begin
          if (!a_busy_q[l] && a_lane_tx_busy_dbg[l]) begin
            ...
            $display("A_AUTOROUTE_TX_ATTEMPT t=%0t pkt=%0d lane=%0d good=%04b",
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (a_tx_done_pulse) begin
        ...
        $display("A_AUTOROUTE_TX_DONE t=%0t count=%0d", $time, tx_done_count + 1);
      end

      if (b_rx_done_pulse) begin
        b_done_count <= b_done_count + 1;
        $display("B_AUTOROUTE_RX_DONE t=%0t count=%0d", $time, b_done_count + 1);
      end

      if (b_rx_valid && b_rx_ready) begin
        if (rx_packet_idx >= PACKET_COUNT) begin
          $fatal(1, "Received unexpected extra autoroute packet byte data=%02x", b_rx_data);
        end
        ...
        if (b_rx_last) begin
          ...
          $display("B_AUTOROUTE_PACKET_OK t=%0t pkt=%0d len=%0d good_src=%04b",
  ...
  task automatic set_route_map(input int map_id);
    begin
      case (map_id)
        0: begin
          ab_map[0] = 2'd0; ab_map[1] = 2'd1; ab_map[2] = 2'd2; ab_map[3] = 2'd3;
          ba_map[0] = 2'd0; ba_map[1] = 2'd1; ba_map[2] = 2'd2; ba_map[3] = 2'd3;
        end
        1: begin
          ab_map[0] = 2'd1; ab_map[1] = 2'd2; ab_map[2] = 2'd3; ab_map[3] = 2'd0;
          ba_map[0] = 2'd2; ba_map[1] = 2'd3; ba_map[2] = 2'd0; ba_map[3] = 2'd1;
        end
        2: begin
          ab_map[0] = 2'd3; ab_map[1] = 2'd2; ab_map[2] = 2'd1; ab_map[3] = 2'd0;
          ba_map[0] = 2'd1; ba_map[1] = 2'd0; ba_map[2] = 2'd3; ba_map[3] = 2'd2;
        end
        default: begin
          ab_map[0] = 2'd2; ab_map[1] = 2'd0; ab_map[2] = 2'd3; ab_map[3] = 2'd1;
          ba_map[0] = 2'd3; ba_map[1] = 2'd1; ba_map[2] = 2'd2; ba_map[3] = 2'd0;
        end
      endcase
      $display("AUTOROUTE_MAP_SET t=%0t id=%0d ab_dst_to_src=%0d,%0d,%0d,%0d ba_dst_to_src=%0d,%0d,%0d,%0d",
        $time, map_id,
        ab_map[0], ab_map[1], ab_map[2], ab_map[3],
        ba_map[0], ba_map[1], ba_map[2], ba_map[3]);
    end
  endtask
  ...
  task automatic send_packet(input int pkt);
    ...
    begin
      ...
      a_tx_last  = 1'b0;
      sent_packets++;
      $display("A_AUTOROUTE_PACKET_QUEUED t=%0t pkt=%0d len=%0d good_src=%04b map=%0d",
      ...
      while (!((rx_packet_idx > pkt) && (tx_done_count > pkt) && (b_done_count > pkt) &&
        ...
        @(posedge clk);
        wait_cycles++;
        if (wait_cycles > 6000000) begin
          $fatal(1,
            "Timeout waiting for autoroute packet completion pkt=%0d rx_pkt=%0d tx_done=%0d b_done=%0d a_active=%0b a_loading=%0b b_rx_ctx=%0b b_rx_complete=%0b a_busy=%04b b_busy=%04b",
      ...
      if ((pkt_a_tx_used[pkt] & packet_good_src_mask[pkt]) == '0) begin
        $fatal(1, "Packet %0d never tried the available route used=%04b good=%04b",
          pkt, pkt_a_tx_used[pkt], packet_good_src_mask[pkt]);
      end
      if (failed_attempts == '0) begin
        $fatal(1, "Packet %0d did not exercise a failed route before success used=%04b good=%04b",
          pkt, pkt_a_tx_used[pkt], packet_good_src_mask[pkt]);
      end
      if (pkt_b_rx_data_used[pkt] != expected_b_rx) begin
        $fatal(1, "Packet %0d A->B autoroute mismatch used=%04b b_rx=%04b expected=%04b good=%04b map=%0d",
      ...
      if (pkt_b_tx_ack_used[pkt] == '0 || pkt_a_rx_ack_used[pkt] == '0) begin
        $fatal(1, "Packet %0d missing ACK route activity b_tx_ack=%04b a_rx_ack=%04b",
      ...
      if ((pkt_a_rx_ack_used[pkt] & expected_a_rx_ack) == '0) begin
        $fatal(1, "Packet %0d B->A ACK route mismatch b_tx_ack=%04b a_rx_ack=%04b expected_any=%04b",
      ...
      $display("AUTOROUTE_PACKET_PASS pkt=%0d map=%0d good_src=%04b tx_attempts=%04b failed_attempts=%04b b_rx_data=%04b b_tx_ack=%04b a_rx_ack=%04b",
  ...
  initial begin
    ...
    enable_a = 1'b0;
    enable_b = 1'b0;
    session_id_a = 16'h7f21;
    session_id_b = 16'h7f21;
    lane_mask_a = '1;
    lane_mask_b = '1;
    ab_good_src_mask = '1;
    ba_good_src_mask = '1;
    ...
    sender_done = 1'b0;
    if (TARGET_RPM != 600 || TARGET_REV_PER_S != 10 || SHAFT_DIAMETER_MM != 200) begin
      $fatal(1, "Rotating stress target metadata mismatch rpm=%0d rev_per_s=%0d diameter_mm=%0d",
    ...
    for (int p = 0; p < PACKET_COUNT; p++) begin
      ...
      packet_map_id[p] = p % SECTORS_PER_REV;
      packet_good_src_mask[p] = 4'b1000 >> ((p + (p / SECTORS_PER_REV)) % SECTORS_PER_REV);
    end

    for (int p = 0; p < PACKET_COUNT; p++) begin
      for (int k = 0; k < MAX_PACKET_BYTES; k++) begin
        expected[p][k] = byte'(8'h90 + (p * 8'h11) + k);
      end
    end
  ...
  initial begin
    repeat (72000000) begin
      ...
      if (rst_n && (a_tx_error_overflow || a_tx_error_retry_exhausted ||
        ...
        $fatal(1,
          "Multi-lane autoroute error flags: a_tx_overflow=%0b a_retry=%0b b_tx_overflow=%0b b_retry=%0b b_hdr=%0b b_proto=%0b b_overflow=%0b b_crc=%0b b_overrun=%0b",
      ...
      if (sender_done && (sent_packets == PACKET_COUNT) && (rx_packet_idx == PACKET_COUNT) &&
        ...
        if (packets_with_failed_attempts != PACKET_COUNT) begin
          $fatal(1, "Not every rotating packet exercised failed-route recovery count=%0d packets=%0d",
            packets_with_failed_attempts, PACKET_COUNT);
        end
        if (good_src_coverage != '1 || tx_attempt_coverage != '1) begin
          $fatal(1, "Rotating autoroute coverage mismatch good=%04b tx_attempt=%04b",
            good_src_coverage, tx_attempt_coverage);
        end
        $display("LOOPBACK_ROTATING_AUTOROUTE_STRESS_PASS rpm=%0d rev_per_s=%0d shaft_diameter_mm=%0d packets=%0d rotations=%0d sectors=%0d good_src_coverage=%04b tx_attempt_coverage=%04b failed_route_packets=%0d",
          TARGET_RPM, TARGET_REV_PER_S, SHAFT_DIAMETER_MM, PACKET_COUNT, ROTATION_COUNT, SECTORS_PER_REV,
          good_src_coverage, tx_attempt_coverage, packets_with_failed_attempts);
        $finish;
      end
    end

    $fatal(1,
      "Timeout waiting for multi-lane autoroute completion sent=%0d rx_pkt=%0d rx_byte=%0d tx_done=%0d b_done=%0d current=%0d active=%0b loading=%0b rx_ctx=%0b complete=%0b",
