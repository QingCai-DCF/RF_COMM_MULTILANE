...
module tb_ir_array_loopback_multi_lane_route;
  ...
  logic [LANE_COUNT-1:0] pkt_b_tx_used [0:PACKET_COUNT-1];
  ...
  always_comb begin
    for (int l = 0; l < LANE_COUNT; l++) begin
      b_ir_rx_in[l] = ~a_ir_tx_out[ab_map[l]];
      a_ir_rx_in[l] = ~b_ir_tx_out[ba_map[l]];
    end
  end
  ...
  always @(negedge clk) begin
    if (!rst_n) begin
      ...
      a_rx_ready  <= 1'b1;
      b_rx_ready  <= !((ready_cycle % 19) == 7 || (ready_cycle % 41) == 13);
    end
  end
      ...
      if ((current_packet_idx >= 0) && (current_packet_idx < PACKET_COUNT)) begin
        for (int l = 0; l < LANE_COUNT; l++) begin
          if (!a_busy_q[l] && a_lane_tx_busy_dbg[l]) begin
            ...
            $display("A_ROUTE_TX_START t=%0t pkt=%0d lane=%0d map=%0d",
              $time, current_packet_idx, l, packet_map_id[current_packet_idx]);
          end
          if (!b_busy_q[l] && b_lane_tx_busy_dbg[l]) begin
            pkt_b_tx_used[current_packet_idx][l] <= 1'b1;
          end
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (a_tx_done_pulse) begin
        ...
        $display("A_ROUTE_TX_DONE t=%0t count=%0d", $time, tx_done_count + 1);
      end

      if (b_rx_done_pulse) begin
        b_done_count <= b_done_count + 1;
        $display("B_ROUTE_RX_DONE t=%0t count=%0d", $time, b_done_count + 1);
      end

      if (b_rx_valid && b_rx_ready) begin
        if (rx_packet_idx >= PACKET_COUNT) begin
          $fatal(1, "Received unexpected extra routed packet byte data=%02x", b_rx_data);
        end
        ...
        if (b_rx_last) begin
          ...
          $display("B_ROUTE_PACKET_OK t=%0t pkt=%0d len=%0d map=%0d",
            $time, rx_packet_idx, packet_len[rx_packet_idx], packet_map_id[rx_packet_idx]);
  ...
  task automatic set_route_map(input int map_id);
    begin
      ...
      endcase
      $display("ROUTE_MAP_SET t=%0t id=%0d ab_dst_to_src=%0d,%0d,%0d,%0d ba_dst_to_src=%0d,%0d,%0d,%0d",
        $time, map_id,
  ...
  task automatic send_packet(input int pkt);
    ...
    begin
      ...
      a_tx_last  = 1'b0;
      sent_packets++;
      $display("A_ROUTE_PACKET_QUEUED t=%0t pkt=%0d len=%0d map=%0d",
        $time, pkt, packet_len[pkt], packet_map_id[pkt]);
      ...
      while (!((rx_packet_idx > pkt) && (tx_done_count > pkt) && (b_done_count > pkt) &&
        ...
        if (wait_cycles > 5000000) begin
          $fatal(1,
            "Timeout waiting for routed packet completion pkt=%0d rx_pkt=%0d tx_done=%0d b_done=%0d a_active=%0b a_loading=%0b b_rx_ctx=%0b b_rx_complete=%0b",
            pkt, rx_packet_idx, tx_done_count, b_done_count,
            a_tx_packet_active, a_tx_packet_loading, b_rx_ctx_valid, b_rx_ctx_complete);
        end
      end

      expected_b_rx = map_ab_mask(pkt_a_tx_used[pkt]);
      if (pkt_b_rx_data_used[pkt] != expected_b_rx) begin
        $fatal(1, "Packet %0d A->B map mismatch tx_used=%04b b_rx_used=%04b expected=%04b map=%0d",
          pkt, pkt_a_tx_used[pkt], pkt_b_rx_data_used[pkt], expected_b_rx, packet_map_id[pkt]);
      end
      if (pkt_a_tx_used[pkt] == '0 || pkt_b_tx_used[pkt] == '0 || pkt_a_rx_ack_used[pkt] == '0) begin
        $fatal(1, "Packet %0d missing route activity a_tx=%04b b_tx_ack=%04b a_rx_ack=%04b",
          pkt, pkt_a_tx_used[pkt], pkt_b_tx_used[pkt], pkt_a_rx_ack_used[pkt]);
      end
      expected_a_rx_ack = map_ba_mask(pkt_b_tx_used[pkt]);
      if ((pkt_a_rx_ack_used[pkt] & expected_a_rx_ack) == '0) begin
        $fatal(1, "Packet %0d B->A ACK route mismatch b_tx_used=%04b a_rx_ack=%04b expected_any=%04b map=%0d",
          pkt, pkt_b_tx_used[pkt], pkt_a_rx_ack_used[pkt], expected_a_rx_ack, packet_map_id[pkt]);
      end

      $display("ROUTE_PACKET_PASS pkt=%0d map=%0d a_tx=%04b b_rx_data=%04b b_tx_ack=%04b a_rx_ack=%04b",
        pkt, packet_map_id[pkt], pkt_a_tx_used[pkt], pkt_b_rx_data_used[pkt],
        pkt_b_tx_used[pkt], pkt_a_rx_ack_used[pkt]);
      current_packet_idx = -1;
      repeat (20) @(posedge clk);
    end
  endtask
  ...
  initial begin
    ...
    enable_a = 1'b0;
    enable_b = 1'b0;
    session_id_a = 16'h7a91;
    session_id_b = 16'h7a91;
    lane_mask_a = '1;
    lane_mask_b = '1;
    a_tx_data = 8'h00;
    a_tx_valid = 1'b0;
    ...
    for (int p = 0; p < PACKET_COUNT; p++) begin
      for (int k = 0; k < MAX_PACKET_BYTES; k++) begin
        expected[p][k] = byte'(8'h60 + (p * 8'h13) + k);
      end
    end
  ...
  initial begin
    repeat (12000000) begin
      ...
      if (rst_n && (a_tx_error_overflow || a_tx_error_retry_exhausted ||
        ...
        $fatal(1,
          "Multi-lane route error flags: a_tx_overflow=%0b a_retry=%0b b_tx_overflow=%0b b_retry=%0b b_hdr=%0b b_proto=%0b b_overflow=%0b b_crc=%0b b_overrun=%0b",
      ...
      if (sender_done && (sent_packets == PACKET_COUNT) && (rx_packet_idx == PACKET_COUNT) &&
          ...
          !a_tx_packet_active && !a_tx_packet_loading && !b_rx_ctx_valid && !b_rx_ctx_complete &&
          (a_lane_tx_busy_dbg == '0) && (b_lane_tx_busy_dbg == '0)) begin
        $display("LOOPBACK_MULTI_LANE_ROUTE_PASS packets=%0d route_maps=%0d,%0d,%0d,%0d,%0d",
          PACKET_COUNT,
          packet_map_id[0], packet_map_id[1], packet_map_id[2], packet_map_id[3], packet_map_id[4]);
        $finish;
      end
    end

    $fatal(1,
      "Timeout waiting for multi-lane route completion sent=%0d rx_pkt=%0d rx_byte=%0d tx_done=%0d b_done=%0d current=%0d active=%0b loading=%0b rx_ctx=%0b complete=%0b",
