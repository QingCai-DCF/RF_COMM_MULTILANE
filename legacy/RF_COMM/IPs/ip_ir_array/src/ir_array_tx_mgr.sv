...
module ir_array_tx_mgr #(
  ...
  parameter int POST_ACK_GUARD_CYCLES = 8192,
  parameter int MAX_INFLIGHT_FRAGS  = 1,
  ...
  localparam int INFLIGHT_W = (MAX_FRAGS <= 1) ? 1 : $clog2(MAX_FRAGS + 1);
  localparam int MAX_INFLIGHT_SAFE =
    (MAX_INFLIGHT_FRAGS < 1) ? 1 :
    ((MAX_INFLIGHT_FRAGS > MAX_FRAGS) ? MAX_FRAGS : MAX_INFLIGHT_FRAGS);
  ...
  logic                         ack_match;
  logic [INFLIGHT_W-1:0]        inflight_count;
  logic [7:0]                   final_probe_frag_idx;
  logic                         final_probe_valid;
  ...
  function automatic logic [7:0] first_valid_frag(input logic [7:0] count);
    integer n;
    begin
      first_valid_frag = 8'h00;
      for (n = 0; n < MAX_FRAGS; n = n + 1) begin
        if ((n < count) && (first_valid_frag == 8'h00)) begin
          first_valid_frag = n[7:0];
        end
      end
    end
  endfunction

  function automatic logic [INFLIGHT_W-1:0] count_frag_bits(input logic [MAX_FRAGS-1:0] bits);
    integer n;
    begin
      count_frag_bits = '0;
      for (n = 0; n < MAX_FRAGS; n = n + 1) begin
        if (bits[n]) count_frag_bits = count_frag_bits + 1'b1;
      end
    end
  endfunction
  ...
  assign ack_match          = ack_valid && packet_active && (ack_session_id == session_id) && (ack_pkt_seq == active_pkt_seq);
  assign inflight_count     = count_frag_bits(frag_inflight);
  assign final_probe_valid  = (frag_count != 0);
  assign final_probe_frag_idx = first_valid_frag(frag_count);
  ...
  always_comb begin
    ...
    if (packet_active && !packet_loading && !issue_valid &&
        (inflight_count < MAX_INFLIGHT_SAFE) && !waiting_final_ack &&
        (post_ack_guard == 32'h0000_0000)) begin
    ...
    if (build_valid) begin
      ...
      build_frame_data[8*11 +: 8] = 8'h00; // reserved in protocol v1
      build_hdr_crc = 16'hFFFF;
  ...
  always_ff @(posedge clk) begin
    ...
    if (!rst_n) begin
      ...
      if (!enable) begin
        ...
        // All fragments are already acknowledged here; a single duplicate fragment is
        // sufficient to make the receiver re-emit the final COMPLETE ACK.
        if (packet_active && waiting_final_ack && !issue_valid && !(ack_match && ack_complete)) begin
          if (final_ack_timeout != 0) begin
            final_ack_timeout <= final_ack_timeout - 1'b1;
          end else if (final_probe_valid && (final_ack_retry_cnt < MAX_RETRY)) begin
            final_ack_retry_cnt             <= final_ack_retry_cnt + 1'b1;
            frag_acked[final_probe_frag_idx]   <= 1'b0;
            frag_inflight[final_probe_frag_idx] <= 1'b0;
            frag_pending[final_probe_frag_idx]  <= 1'b1;
            waiting_final_ack <= 1'b0;
            final_ack_timeout <= FRAG_TIMEOUT_CYCLES;
          end else begin
        ...
        if (packet_active) begin
          for (i = 0; i < MAX_FRAGS; i = i + 1) begin
            if ((i < frag_count) && frag_inflight[i] && !frag_acked[i] && !(ack_match && ack_bitmap[i])) begin
