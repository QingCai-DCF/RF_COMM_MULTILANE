module ir_array_top #(
  ...
  output logic [31:0]                  rx_debug_status_dbg
);
  ...
  logic [RR_W-1:0] tx_sel_lane;
  logic [RR_W-1:0] rx_sel_lane;
  logic            tx_sel_valid;
  logic            rx_sel_valid;
  ...
  ir_array_rx_mgr #(
    ...
    .rx_debug_status      (rx_debug_status_dbg)
  );
  ...
  generate
    for (gi = 0; gi < LANE_COUNT; gi = gi + 1) begin : g_lane
      ir_comm_lane #(
        ...
        .clk              (clk_phy),
        .rst_n            (rst_n),
        .tx_enable        (enable && lane_enable_mask[gi]),
        .rx_enable        (enable && rx_lane_enable_mask[gi]),
        ...
        .rx_debug_status  (lane_rx_debug_status_dbg[32*gi +: 32])
      );
    end
  endgenerate

  assign lane_tx_busy_dbg      = lane_busy;
  assign lane_tx_load_pulse_dbg = lane_load;
  assign lane_rx_frame_pulse_dbg = lane_rx_frame_valid & lane_rx_frame_ready;
  assign lane_rx_crc_error_dbg = lane_rx_crc_error;
  assign lane_rx_error_dbg = lane_rx_frame_overflow | lane_rx_overrun_error;
  assign rx_frame_overflow_any = |lane_rx_frame_overflow;
  ...
  always_comb begin
    ...
    for (tx_l = 0; tx_l < LANE_COUNT; tx_l = tx_l + 1) begin
      tx_cand = tx_rr_ptr + tx_l;
      if (tx_cand >= LANE_COUNT) tx_cand = tx_cand - LANE_COUNT;
      if (!tx_sel_valid && (rx_to_tx_guard == 32'h0000_0000) &&
          lane_enable_mask[tx_cand] && lane_load_ready[tx_cand]) begin
        tx_sel_valid = 1'b1;
        tx_sel_lane  = tx_cand[RR_W-1:0];
      end
    end

    if (tx_sel_valid) begin
  ...
  always_comb begin
    ...
    for (rx_l = 0; rx_l < LANE_COUNT; rx_l = rx_l + 1) begin
      rx_cand = rx_rr_ptr + rx_l;
      if (rx_cand >= LANE_COUNT) rx_cand = rx_cand - LANE_COUNT;
      if (!rx_sel_valid && lane_rx_frame_valid[rx_cand]) begin
        rx_sel_valid = 1'b1;
        rx_sel_lane  = rx_cand[RR_W-1:0];
      end
    end
    ...
    if (rx_sel_valid) begin
      ...
      ingress_lane_id    = {{(8-RR_W){1'b0}}, rx_sel_lane};
  ...
  always_ff @(posedge clk_phy) begin
    if (!rst_n) begin
      ...
      if (tx_sel_valid && ((ack_issue_valid && ack_issue_ready) || (tx_issue_valid && tx_issue_ready))) begin
        if (tx_sel_lane == LANE_COUNT-1) tx_rr_ptr <= '0;
        else                             tx_rr_ptr <= tx_sel_lane + 1'b1;
      end
      if (rx_sel_valid && ingress_valid && ingress_ready) begin
        rx_to_tx_guard <= RX_TO_TX_GUARD_CYCLES[31:0];
        if (rx_sel_lane == LANE_COUNT-1) rx_rr_ptr <= '0;
        else                             rx_rr_ptr <= rx_sel_lane + 1'b1;
      end
    end
  end
endmodule
