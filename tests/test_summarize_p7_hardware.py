...
class SummarizeP7HardwareTests(unittest.TestCase):
    ...
    def test_p6_jtag_candidate_binds_axi4_bursts_through_axi4lite_converter(self) -> None:
        ...
        converter_properties = (ROOT / "evidence/generated/vivado/p6_ip_inspect/axi_protocol_converter_properties.txt").read_text(encoding="utf-8")
        top = (ROOT / "rtl/p6_jtag_top.sv").read_text(encoding="utf-8")
        stage_tcl = (ROOT / "scripts/hw/p7_jtag_axi_transactions.tcl").read_text(encoding="utf-8")
        summary = json.loads(
        ...
        for text in (build_tcl, inspect_tcl):
            self.assertIn("CONFIG.PROTOCOL {0}", text)
            self.assertIn("CONFIG.RD_TXN_QUEUE_LENGTH {16}", text)
            self.assertIn("CONFIG.WR_TXN_QUEUE_LENGTH {16}", text)
            self.assertIn("CONFIG.SI_PROTOCOL {AXI4}", text)
            self.assertIn("CONFIG.MI_PROTOCOL {AXI4LITE}", text)
        self.assertIn("CONFIG.PROTOCOL=0", properties)
        self.assertIn("CONFIG.RD_TXN_QUEUE_LENGTH=16", properties)
        self.assertIn("CONFIG.WR_TXN_QUEUE_LENGTH=16", properties)
        self.assertIn("CONFIG.SI_PROTOCOL=AXI4", converter_properties)
        self.assertIn("CONFIG.MI_PROTOCOL=AXI4LITE", converter_properties)
        self.assertIn("p6_axi_protocol_converter u_axi_protocol_converter", top)
        self.assertEqual(1, stage_tcl.count("proc p7_axi_write_burst"))
        self.assertEqual(1, stage_tcl.count("proc p7_axi_read_burst"))
        self.assertEqual(2, stage_tcl.count("-burst INCR"))
    ...
    def test_real_r1_through_r29_hardware_epochs_validate_and_failed_stage_tamper_fails_closed(self) -> None:
        ...
        epoch_names = (
            ...
            "p7_20260711_stationary_app_r13_diag_suffix55",
            "p7_20260711_stationary_app_r14_diag_suffix55",
            "p7_20260711_stationary_app_r15_diag_suffix55",
            "p7_20260711_stationary_app_r16_diag_suffix55",
            "p7_20260712_stationary_app_r18_diag_suffix62",
            "p7_20260712_stationary_app_r21_diag_suffix62",
            "p7_20260712_stationary_app_r22_diag_suffix62",
            "p7_20260712_stationary_app_r23_diag_suffix62",
            "p7_20260712_stationary_app_r24_diag_suffix62",
            "p7_20260712_stationary_app_r25_diag_suffix62",
            "p7_20260712_stationary_app_r26_diag_suffix55",
            "p7_20260712_stationary_app_r27_diag_suffix62",
        ...
        for epoch_name in epoch_names:
            stage_directory = (
                ...
                if epoch_name.endswith(("_r16_diag_suffix55", "_r18_diag_suffix62", "_r21_diag_suffix62", "_r22_diag_suffix62", "_r23_diag_suffix62", "_r24_diag_suffix62", "_r25_diag_suffix62", "_r26_diag_suffix55", "_r27_diag_suffix62", "_r28_diag_suffix55", "_r29_diag_suffix55"))
                ...
                if epoch_name.endswith(("_r11_diag_suffix55", "_r12_diag_suffix55", "_r14_diag_suffix55", "_r15_diag_suffix55"))
            ...
            candidate_kind = "ps" if epoch_name.endswith(("_r16_diag_suffix55", "_r18_diag_suffix62", "_r21_diag_suffix62", "_r22_diag_suffix62", "_r23_diag_suffix62", "_r24_diag_suffix62", "_r25_diag_suffix62", "_r26_diag_suffix55", "_r27_diag_suffix62", "_r28_diag_suffix55", "_r29_diag_suffix55")) else "jtag"
            candidate = subject.Candidate(
                summary_path,
                data,
                candidate_kind,
                subject.classify_ps_stage(data) if candidate_kind == "ps" else subject.classify_jtag_stage(data),
                subject.parse_time(data.get("generated_at_utc"), summary_path.stat().st_mtime),
            )
            candidates[epoch_name] = candidate
            if epoch_name.endswith(("_r23_diag_suffix62", "_r24_diag_suffix62", "_r25_diag_suffix62", "_r26_diag_suffix55", "_r27_diag_suffix62", "_r28_diag_suffix55", "_r29_diag_suffix55")):
            ...
            elif epoch_name.endswith("_r16_diag_suffix55"):
                inner_errors = subject._old_commit_ps_shutdown_arg_count_failure_errors(candidate, evidence)
            elif epoch_name.endswith("_r10_diag_suffix55"):
                inner_errors = subject._old_commit_1m_jtag_timeout_errors(candidate, evidence)
            elif epoch_name.endswith("_r13_diag_suffix55"):
                inner_errors = subject._old_commit_1m_jtag_queued_timeout_errors(candidate, evidence)
            elif epoch_name.endswith("_r14_diag_suffix55"):
                inner_errors = subject._old_commit_axi4_queued_control_order_failure_errors(candidate, evidence)
            elif epoch_name.endswith("_r15_diag_suffix55"):
                inner_errors = subject._old_commit_axi4_burst_word_order_failure_errors(candidate, evidence)
            ...
            if epoch_name.endswith(("_r26_diag_suffix55", "_r28_diag_suffix55", "_r29_diag_suffix55")):
            ...
            if epoch_name.endswith("_r15_diag_suffix55"):
                self.assertEqual(
                    20,
                    epoch["axi4_burst_word_order_rejection_failure"][
                        "reversed_word_crc_fragment_proof_count"
                    ],
                )
        ...
        r13 = candidates["p7_20260711_stationary_app_r13_diag_suffix55"]
        r13_tamper_cases = (
            ("timeout fact", lambda data: data["stage_process"].__setitem__("timed_out", False)),
            ("queue batch count", lambda data: data["transaction_validation"].__setitem__("multi_transaction_batch_count", 0)),
            ("candidate programming", lambda data: data.__setitem__("programmed_candidate", False)),
        ...
        for label, mutate in r13_tamper_cases:
            tampered_data = json.loads(json.dumps(r13.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r13.path, tampered_data, r13.kind, r13.stage, r13.timestamp)
            errors = subject._old_commit_1m_jtag_queued_timeout_errors(tampered, evidence)
            self.assertTrue(errors, f"r13 {label} tamper unexpectedly validated")

        r14 = candidates["p7_20260711_stationary_app_r14_diag_suffix55"]
        r14_tamper_cases = (
            ("backend failure", lambda data: data.__setitem__("backend_parse_failure", "")),
            ("AXI4 burst count", lambda data: data["transaction_validation"].__setitem__("axi4_incr_burst_count", 0)),
            ("candidate programming", lambda data: data.__setitem__("programmed_candidate", False)),
            ("shutdown-after", lambda data: data["shutdown_after"].__setitem__("passed", False)),
            ("hardware promotion", lambda data: data.__setitem__("hardware_acceptance", "PASS")),
        )
        for label, mutate in r14_tamper_cases:
            tampered_data = json.loads(json.dumps(r14.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r14.path, tampered_data, r14.kind, r14.stage, r14.timestamp)
            errors = subject._old_commit_axi4_queued_control_order_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r14 {label} tamper unexpectedly validated")

        r15 = candidates["p7_20260711_stationary_app_r15_diag_suffix55"]
        r15_tamper_cases = (
            ("backend failure", lambda data: data.__setitem__("backend_parse_failure", "")),
            ("ordered-control metric", lambda data: data["transaction_validation"].__setitem__("ordered_control_write_run_hw_axi_call_count", 0)),
            ("candidate programming", lambda data: data.__setitem__("programmed_candidate", False)),
            ("shutdown-after", lambda data: data["shutdown_after"].__setitem__("passed", False)),
            ("hardware promotion", lambda data: data.__setitem__("hardware_acceptance", "PASS")),
        )
        for label, mutate in r15_tamper_cases:
            tampered_data = json.loads(json.dumps(r15.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r15.path, tampered_data, r15.kind, r15.stage, r15.timestamp)
            errors = subject._old_commit_axi4_burst_word_order_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r15 {label} tamper unexpectedly validated")

        r16 = candidates["p7_20260711_stationary_app_r16_diag_suffix55"]
        r16_tamper_cases = (
            ("shutdown-before return code", lambda data: data["shutdown_before"].__setitem__("returncode", 0)),
            ("candidate programming", lambda data: data.__setitem__("programmed_candidate", True)),
            ("PS ELF start", lambda data: data.__setitem__("started_ps_elf", True)),
            ("shutdown-after reap", lambda data: data["shutdown_after"].__setitem__("process_tree_reaped", False)),
            ("hardware promotion", lambda data: data.__setitem__("HARDWARE_ACCEPTANCE", "PASS")),
        )
        for label, mutate in r16_tamper_cases:
            tampered_data = json.loads(json.dumps(r16.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r16.path, tampered_data, r16.kind, r16.stage, r16.timestamp)
            errors = subject._old_commit_ps_shutdown_arg_count_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r16 {label} tamper unexpectedly validated")

        r18 = candidates["p7_20260712_stationary_app_r18_diag_suffix62"]
        r18_tamper_cases = (
            ("candidate programming", lambda data: data.__setitem__("programmed_candidate", True)),
            ("PS ELF start", lambda data: data.__setitem__("started_ps_elf", True)),
            ("raw failure class", lambda data: data["ps_process"].__setitem__("failures", [])),
            ("shutdown-after reap", lambda data: data["shutdown_after"].__setitem__("process_tree_reaped", False)),
            ("hardware promotion", lambda data: data.__setitem__("HARDWARE_ACCEPTANCE", "PASS")),
        )
        for label, mutate in r18_tamper_cases:
            tampered_data = json.loads(json.dumps(r18.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r18.path, tampered_data, r18.kind, r18.stage, r18.timestamp)
            errors = subject._old_commit_ps_reset_target_uniqueness_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r18 {label} tamper unexpectedly validated")

        r21 = candidates["p7_20260712_stationary_app_r21_diag_suffix62"]
        for label, mutate in r18_tamper_cases:
            tampered_data = json.loads(json.dumps(r21.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r21.path, tampered_data, r21.kind, r21.stage, r21.timestamp)
            errors = subject._old_commit_ps_reset_target_uniqueness_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r21 {label} tamper unexpectedly validated")

        r22 = candidates["p7_20260712_stationary_app_r22_diag_suffix62"]
        for label, mutate in r18_tamper_cases:
            tampered_data = json.loads(json.dumps(r22.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r22.path, tampered_data, r22.kind, r22.stage, r22.timestamp)
            errors = subject._old_commit_ps_reset_target_uniqueness_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r22 {label} tamper unexpectedly validated")

        r23 = candidates["p7_20260712_stationary_app_r23_diag_suffix62"]
        r23_tamper_cases = (
            ("candidate programming", lambda data: data.__setitem__("programmed_candidate", False)),
            ("PS ELF start", lambda data: data.__setitem__("started_ps_elf", False)),
            ("PS failure list", lambda data: data.__setitem__("ps_failures", [])),
            ("shutdown-after", lambda data: data["shutdown_after"].__setitem__("passed", False)),
            ("hardware promotion", lambda data: data.__setitem__("HARDWARE_ACCEPTANCE", "PASS")),
        )
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r23.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r23.path, tampered_data, r23.kind, r23.stage, r23.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r23 {label} tamper unexpectedly validated")

        r24 = candidates["p7_20260712_stationary_app_r24_diag_suffix62"]
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r24.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r24.path, tampered_data, r24.kind, r24.stage, r24.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r24 {label} tamper unexpectedly validated")

        r25 = candidates["p7_20260712_stationary_app_r25_diag_suffix62"]
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r25.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r25.path, tampered_data, r25.kind, r25.stage, r25.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r25 {label} tamper unexpectedly validated")

        r26 = candidates["p7_20260712_stationary_app_r26_diag_suffix55"]
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r26.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r26.path, tampered_data, r26.kind, r26.stage, r26.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r26 {label} tamper unexpectedly validated")

        r27 = candidates["p7_20260712_stationary_app_r27_diag_suffix62"]
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r27.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r27.path, tampered_data, r27.kind, r27.stage, r27.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r27 {label} tamper unexpectedly validated")

        r28 = candidates["p7_20260712_stationary_app_r28_diag_suffix55"]
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r28.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r28.path, tampered_data, r28.kind, r28.stage, r28.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r28 {label} tamper unexpectedly validated")

        r29 = candidates["p7_20260712_stationary_app_r29_diag_suffix55"]
        for label, mutate in r23_tamper_cases:
            tampered_data = json.loads(json.dumps(r29.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r29.path, tampered_data, r29.kind, r29.stage, r29.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r29 {label} tamper unexpectedly validated")
