...
class SummarizeP7HardwareTests(unittest.TestCase):
    ...
    def test_real_r1_through_r31_hardware_epochs_validate_and_failed_stage_tamper_fails_closed(self) -> None:
        ...
        r30 = candidates["p7_20260712_stationary_app_r30_diag_suffix55"]
        r30_tamper_cases = r23_tamper_cases + (
            (
                "source commit",
                lambda data: data["safety_validation"].__setitem__(
                    "source_commit_requested", "0" * 40
                ),
            ),
        )
        for label, mutate in r30_tamper_cases:
            tampered_data = json.loads(json.dumps(r30.data))
            mutate(tampered_data)
            tampered = subject.Candidate(r30.path, tampered_data, r30.kind, r30.stage, r30.timestamp)
            errors = subject._old_commit_ps_functional_boundary_failure_errors(tampered, evidence)
            self.assertTrue(errors, f"r30 {label} tamper unexpectedly validated")

        original_read_bytes = Path.read_bytes
        for target_name in (
            "boundary_10_integrity_snapshot_failure.bin",
            "boundary_10_integrity_snapshot_wipe_verify.bin",
        ):
            def tampered_read_bytes(path: Path, *, _target: str = target_name) -> bytes:
            ...
            with mock.patch.object(Path, "read_bytes", new=tampered_read_bytes):
                errors = subject._old_commit_ps_functional_boundary_failure_errors(r30, evidence)
            self.assertIn(
                "historical r30 exact OCM failure snapshot/wipe evidence mismatch",
                errors,
