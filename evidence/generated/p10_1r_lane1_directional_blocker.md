...
The frozen P10.1R bundle currently passes the raw lane1 direction `R1 -> F1` but fails `F1 -> R1`. The failure is reproducible from a fresh boot and does not require preceding lane0 traffic. Automated reset/reprogram retries cannot distinguish or repair the remaining physical boundary, and this campaign forbids Codex from moving, aligning, obstructing, exchanging, or rewiring the hardware.
...
| `F1 -> R1` | `FAIL_NO_REMOTE_RAW_ACTIVITY` | On sample 0, fixed completed with status `0x0`, state `4`; rotating returned `0xE` (`P9_RUNTIME_RAW_TIMEOUT`), state `5`; both PHY status and error-detail registers were zero. |
...
The fixed endpoint completes its F1 raw command while the rotating endpoint sees no R1 raw event. Together with the working reverse direction, the unresolved boundary is the current fixed-F1 transmit/emission/optical-path-to-rotating-R1 receive/detection chain.

Without user physical inspection or external electrical/optical evidence, the following remain indistinguishable:

- fixed F1 emitter/module power/SD/Mode/Txd/connector/board-output path;
...
See [final shutdown record](../../evidence/hardware/p10_1r/p10_1r_20260802T182921Z_cce2180b_c1370686_bfb1c51d/shutdown/finally_emergency/attempt_1.result.txt), SHA256 `e9cae8f81d93ea6a36e424ae37379b433920658e2cc0c6206f996d7c20d3f065`.
