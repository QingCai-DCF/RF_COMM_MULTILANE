set log_file {C:/Users/user/Documents/RF_COMM_MULTILANE/evidence/hardware/p5/safe_idle_recheck/p5_safe_idle_recheck_programming.log}
set ila_dir {C:/Users/user/Documents/RF_COMM_MULTILANE/evidence/hardware/p5/ila/safe_idle_recheck}
file mkdir $ila_dir
...
say "P5_SAFE_IDLE_RECHECK_PROGRAM_BEGIN [clock format [clock seconds] -format %Y-%m-%dT%H:%M:%S%z]"
set bit_file {C:/Users/user/Documents/RF_COMM_MULTILANE/evidence/generated/vivado/ir_top_new_safe_idle.bit}
if {![file exists $bit_file]} {
  say "P5_SAFE_IDLE_RECHECK_BITSTREAM_MISSING=$bit_file"
  close $fh
  exit 20
}
set ltx_file {C:/Users/user/Documents/RF_COMM_MULTILANE/evidence/generated/vivado/p4_auto_safe_idle_debug.ltx}
set rc [catch {
  open_hw_manager
  ...
  foreach prop {NAME PART IDCODE IS_PROGRAMMED PROGRAM.FILE} {
    if {[catch {set value [get_property $prop $dev]} prop_err]} {
      say "P5_SAFE_IDLE_RECHECK_DEVICE_PROP $prop ERROR=$prop_err"
    } else {
      say "P5_SAFE_IDLE_RECHECK_DEVICE_PROP $prop=$value"
    }
  }
  set_property PROGRAM.FILE $bit_file $dev
  if {$ltx_file ne "" && [file exists $ltx_file]} {
    set_property PROBES.FILE $ltx_file $dev
    say "P5_SAFE_IDLE_RECHECK_PROBES_FILE=$ltx_file"
  } else {
    say "P5_SAFE_IDLE_RECHECK_PROBES_FILE=MISSING"
  }
  ...
  say "P5_SAFE_IDLE_RECHECK_BITSTREAM_PROGRAMMED=$bit_file"
  ...
  set capture_idx 0
  foreach ila $ilas {
    set wdb_file [file join $ila_dir "safe_idle_recheck_${capture_idx}.wdb"]
    set csv_file [file join $ila_dir "safe_idle_recheck_${capture_idx}.csv"]
    set ila_rc [catch {
      current_hw_ila $ila
  ...
  say "P5_SAFE_IDLE_RECHECK_PROGRAM=PASS"
  catch {close_hw_target}
  catch {disconnect_hw_server}
  catch {close_hw_manager}
} err opts]
if {$rc != 0} {
  say "P5_SAFE_IDLE_RECHECK_PROGRAM=FAIL"
  say "P5_SAFE_IDLE_RECHECK_PROGRAM_ERROR=$err"
  catch {close_hw_target}
  catch {disconnect_hw_server}
  catch {close_hw_manager}
  close $fh
