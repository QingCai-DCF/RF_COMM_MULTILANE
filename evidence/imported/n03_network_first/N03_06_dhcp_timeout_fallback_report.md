...
Source/static checks confirm DHCP start/timeout/static fallback plumbing. Latest read-only UART probe verdict: `INCONCLUSIVE_NO_UART_TEXT` with 0 captured bytes. Safe wrapper DHCP fallback pass=0. Real pass requires UART DHCP timeout/static IP/TCP_READY plus real TCP HELLO and memory echo in the safe wrapper.
