...
The live JTAG master is AXI4-Lite and therefore every hardware transaction must remain `LEN=1`; `INCR` bursts are forbidden. The authorization-bound Tcl executor may batch only naturally consecutive words in the TX payload window (`0x200..0x2fc`) and the TX/RX readback windows (`0x200..0x3fc`) by creating at most 16 ordered single-word transactions and submitting them with `run_hw_axi -queue`. The active JTAG_AXI IP must itself be built with both `CONFIG.RD_TXN_QUEUE_LENGTH=16` and `CONFIG.WR_TXN_QUEUE_LENGTH=16`; the content-addressed candidate build summary and an offline Vivado property inspection must bind those values before a queued hardware run is authorized. A tool-level queue limit does not override a target IP configured with queue depth 1. The executor must keep the original DSL operation count, AXI transfer order, validation order, per-word result keys, and strict backend evidence unchanged. Fixed control/status accesses and polls remain standalone calls. Dry validation records the minimum single-word AXI4-Lite transaction count, minimum `run_hw_axi` call count, multi-transaction batches, batched words, and maximum batch size; it never reports a batch as a burst or as fewer AXI4-Lite transactions. No performance optimization may increase the 1800-second authorization ceiling.
